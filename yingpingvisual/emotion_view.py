...
def emotionview(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if movie_type:
                mte = movietypeem(movie_type)
                content = dict(
                    host=LOC_HOST,
                    mteechart=mte.render_embed(),
                    mte_script=mte.get_js_dependencies()
                )
    ...
    content = dict(
        ...
        st_script=st.get_js_dependencies(),
        mteechart = mte.render_embed(),
        mte_script = mte.get_js_dependencies(),
    )
    return HttpResponse(template.render(content,request))
...
def movietypeem(movietype='传记'):
    ...
    for tkey, tvalue in mtcrresult.items():
        ...
        if type:
            pie.add("", [tkey, "好评", "中评", "差评"], [tvalue[3], tvalue[0], tvalue[1], tvalue[2]],is_label_show=True,)#legend_top="bottom")
    page.add(pie)
    return page
